...
if response.status_code == 200:
  ...
  #plt.plot(df_base['date'], df_base['value'], linestyle='--',color = 'gray',label="Federal")
...
for count,i in enumerate(series_identifiers) :
    ...
    if response.status_code == 200: # Successful Request, all other codes indicated error in payload/server-side
        ...
        df['value'][df['value']<0] = 0  # keep only values above federal
        ...
        y_offset = 0.2  # vertical shift to avoid overlap ~~ FROM GPT
...
plt.title(f"State-Wise Minimum Wage Rates of {', '.join(state_labels)} above the federal minimum: (1968 to 2025)")
plt.xlabel("Year")
...
plt.savefig(f"assets/Wage_{timestamp}.png")
...
app.layout = html.Div([
    ...
    html.Img(src=f'/assets/Wage_{timestamp}.png')
])
